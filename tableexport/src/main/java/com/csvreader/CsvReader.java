...
public class CsvReader {
     ...
     * Default is combination of standard end of line characters for Windows,
     * Unix, or Mac.
     */
     ...
     * during parsing, normally resulting from improper data format.
     */
        ...
        if (startedColumn) {
            if (columnBuffer.Position == 0) {
                if (dataBuffer.ColumnStart < dataBuffer.Position) {
                    int ll = dataBuffer.Position - 1;

                    if (userSettings.TrimWhitespace && !startedWithQualifier) {
                        while (ll >= dataBuffer.ColumnStart
                                && (dataBuffer.Buffer[ll] == Letters.SPACE
                                || dataBuffer.Buffer[ll] == Letters.TAB)) {
                            ll--;
                        }
                    }

                    currentValue = new String(dataBuffer.Buffer,
                            dataBuffer.ColumnStart, ll
                            - dataBuffer.ColumnStart + 1);
                }
            } else {
                updateCurrentValue();

                int ll = columnBuffer.Position - 1;

                if (userSettings.TrimWhitespace && !startedWithQualifier) {
                    while (ll >= 0
                            && (columnBuffer.Buffer[ll] == Letters.SPACE
                            || columnBuffer.Buffer[ll] == Letters.SPACE)) {
                        ll--;
                    }
                }
    ...
    protected void finalize() {
        try {
            ...
            try {
                ...
                Logger.getLogger(CsvReader.class.getName()).log(Level.SEVERE,
                        null, ex);
            }
        }
    }
